...
export function lightUpdates({
  ...
  setEntities,
  serviceData,
}: ServiceArgs<'light', 'turnOn' | 'turn_on' | 'toggle'>): boolean | void {
  const dates = {
    last_changed: now,
    last_updated: now,
  };
  ...
  switch (service) {
    ...
    case 'turnOn':
      return setEntities(entities => {
        ...
        if (serviceData?.rgbww_color) {
          ...
          attributes.rgbww_color = serviceData.rgbww_color as [
            number,
            number,
            number,
            number,
          ...
          attributes.rgbw_color = serviceData.rgbw_color as [
            number,
            number,
            number,
            number,
          ...
          attributes.rgb_color = serviceData.rgb_color as [
            number,
            number,
            number,
          ];
    ...
    case 'toggle':
      return setEntities(entities => {
        ...
        if (prev.state === 'on') newState = 'off';
        if (prev.state === 'off') newState = 'on';
        return {
          ...entities,
          [target]: {
            ...prev,
